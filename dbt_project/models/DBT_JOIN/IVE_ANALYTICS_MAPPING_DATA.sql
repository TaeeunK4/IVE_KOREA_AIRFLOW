SELECT
    ...
    A."RPT_TYPE", A."MDA_ARR",  A."TURN_REAL"/ NULLIF(A."TURN_GOAL", 0) AS "ABS", I."INDUSTRY",
    CASE
        WHEN A."TURN_GOAL" = 100000 THEN 'UNLIMITED'
        WHEN A."TURN_GOAL" <> 100000 THEN 'LIMITED'
        END AS "LIMIT_TYPE"
FROM {{ref('IVE_ANALYTICS_DATA')}} AS A
INNER JOIN {{source('s3_mapping_data','IVE_INDUSTRY_MAPPING')}} AS I ON A."NAME"=I."NAME"
WHERE
    A."TIME_CLK" IS NOT NULL AND A."TIME_TURN" IS NOT NULL AND A."ADV_COST" IS NOT NULL AND A."1000_W_EFFICIENCY" IS NOT NULL
